...
for (let i = 0; i < checkboxProduct.length; i++) {
  checkboxProduct[i].addEventListener('click', () => {
    const selectProduct = document.querySelector('input[name="select-product"]');
    ...
    const totalPriceSelectProduct = (Number(price.replace(',', '')) + Number(totalPrice.innerHTML)).toLocaleString('en-US')
    selectProduct.checked ? totalPrice.innerHTML = totalPriceSelectProduct : totalPrice.innerHTML = totalPriceInner;
  })
}
